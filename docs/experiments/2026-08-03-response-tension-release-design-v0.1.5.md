...
## Frozen next-experiment packet v0.1.5
...
| Schema | `edcm.response-tension-release-experiment/0.1.5` |
| Candidate | `edcm.response-tension-release/0.1.0` |
| Canon selection | `null` |
| Supersedes | Immutable v0.1.4 packet SHA-256 `7f84af9c13e20d07069db186ca32d7284b0bbc849ca3b24db755b42e4285fd40` |
| Current-packet index | `docs/experiments/README.md` |

This complete v0.1.5 packet freezes the same single new experiment design and
supersedes v0.1.4 only to align loss-recorded source-whitespace normalization
with the maintained parser and to classify invalid development or validation
scores as incomplete nonconformance rather than candidate falsification. It
does not modify any immutable earlier packet, authorize execution, alter EDCM
equations, reopen prior evidence, activate production, or select a canonical
measurement. No earlier packet was executed; no candidate input, human label,
partition membership, per-event score, or sealed outcome was viewed between
packet versions.

This v0.1.5 packet is immutable. Named people, keys, encrypted-bundle digests,
...
| PR head after transitive-runtime provenance repair | `2f361f07247f8714c3b0648925bb3b99d0ab1eb5` |
| PR head tree | `db52e32f6bd4e08ac44c746cded1b6bf1c6f43a1` |
| Actions synthetic merge | `d59280cee4e5f4468e522c3b4fa91785ba9b0352` |
...
|---|---|---|
| [UCNS–EDCM joint experiments](https://github.com/The-Interdependency/edcm/actions/runs/30790136104) | Python 3.11/3.12 historical experiment tests and byte-identical v0.1–v0.4 reruns | Historical joint reports; not a raw-corpus rerun of the new holdout |
| [Skill and metadata compliance](https://github.com/The-Interdependency/edcm/actions/runs/30790136079) | Seven pinned skill-lib consumers clean; canonical metadata regeneration matched | Contract and drift evidence only |
| [CI](https://github.com/The-Interdependency/edcm/actions/runs/30790136102) | 243 tests passed with 20 producer-dependent skips on Python 3.11–3.13; integrity, authority boundaries, packaging, Twine, and clean-wheel smoke passed | Raw MultiWOZ bytes and event labels were absent; the sealed evaluation was not independently regenerated |
...
the loaded transitive score path, including `metrics.compute`'s bound stats and
risk helpers and `metrics.risk`'s bound stats globals, against the expected
`metrics/stats.py` and `metrics/risk.py` sources; mixed preloaded helper
bindings fail closed. It leaves a single-run byte-repeat finding
...
transform. For each source turn, compute these values in order under the frozen
Python interpreter:
...
values; otherwise fail closed. Speaker prefixes are framing and must not
remain in `Turn.text` or its tokens. Ignore the parser's cycle-grouped
`rounds` for this candidate.
...
as ASCII, concatenate the rows without a header, and record the SHA-256.
Duplicate event digests, missing turn indexes, or non-contiguous indexes fail
closed. Hex digests are lowercase. Every decimal field is unsigned base-10
ASCII without leading zeros, with zero encoded exactly as `0`. For each
partition, provenance records this
ledger digest, its row count, turns affected by CR, LF, leading trim, and
trailing trim, plus total CR/LF replacements and leading/trailing code points
removed. The digest and aggregate counts may leave custody; ledger rows, raw
padding, event digests, per-turn digests, and source text may not.
...
source act contains exactly one of `Booking-Book` and `Booking-NoBook`. Every
source turn through that response must remain nonempty under the frozen
presentation transform and pass the exact framing/parser reconciliation above;
an empty turn is excluded and counted as `empty-after-normalization`.
The transform audit and structural exclusions occur before the eligible
inventory freezes or any outcome label is created. The source act is a locator
only. There is at most one event per dialogue. Exact post-transform framed-input
duplicates are deduplicated before allocation. No dialogue or post-transform
framed-input digest may cross partitions.
...
3. Author and freeze the manual and digest-excluded training examples while
   score-blind; freeze source, presentation-transform, eligibility, exclusion, and
   allocation-code identities and audits; generate the one allocation seed and
   publish its signed commitment before any label is created.
...
5. Commit encrypted bundle, label ledger, access-log, post-transform
   input-inventory digests, ordered partition-manifest digests, and each
   partition's whitespace-loss ledger digest and counts, all bound to the
   pre-label seed commitment and before any development release.
...
conformance, the score must have non-zero
variance and development area under the receiver-operating-characteristic
curve at least `0.55` in the declared direction. Development AUC is the exact
...
score contributes `0`, divided by the number of pairs. Zero variance or
sub-threshold AUC is a preserved development falsification and stops the
...
records the ordered score-inventory digest, variance, AUC method, AUC value,
declared direction, gate thresholds, and gate verdict even when that verdict
...
the selected threshold must be at least
`0.55`, sensitivity at least `0.50`, and specificity at least `0.50`. Failure
of a conforming metric gate is a preserved validation falsification and stops
the experiment before test.
...
The per-partition whitespace-loss ledger digests and aggregate loss counts may
also leave custody; their ledger rows, raw padding, and event digests do not.
...
- frozen candidate identity or synthetic conformance fails before development
  release;
- the presentation transform or parser reconciliation fails after inventory
  freeze, including an admitted turn that is empty post-transform;
...
| Candidate | candidate ID, exact newline/edge-trim and framing rules, source/post-transform turn-digest and whitespace-loss ledger rule, ledger digest and aggregate CR/LF/edge-loss counts, parser reconciliation verdict, single parser call and arguments, one-turn construction, formula, direction, `alpha`, `delta_max`, code commit/tree/blob identities |
...
| Eligibility | rule version, old-test exclusion digest, post-transform input digest chain and duplicate audit, whitespace-loss counts/ledger digest, `empty-after-normalization` exclusions, eligible/excluded counts and reasons |
...
2. one contract-test file covering four synthetic tension transitions; exact
   CR/LF-before-trim order, left/right padded-turn normalization, preservation
   of internal whitespace, parser reconciliation, source/post-transform turn
   digests, whitespace-loss counts, and ledger determinism,
   post-transform duplicate isolation, empty-turn exclusion, exact framing,
   and one-turn construction; response inclusion and label/future-turn
   exclusion; old-test exclusion and partition isolation; equality at the
   `>=` threshold; manual-author role/score blindness; both reliability gates;
   pre-label seed binding; complete development-gate provenance; missing
   outputs, extra or unexpected event-output rows, duplicate event-output rows,
   NaN, positive/negative-infinity, below-zero, and above-one outputs in
   development, validation, and test as incomplete with scientific gates not
   evaluated; equal numeric scores on distinct events as valid; zero variance,
   sub-threshold AUC, and valid-score validation metric failures as
   `stopped-before-test`; sealed ordered score-inventory digests;
   aggregate-only output, fail-closed behavior, and two-run determinism;
