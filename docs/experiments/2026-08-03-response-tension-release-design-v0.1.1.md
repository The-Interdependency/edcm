...
## Frozen next-experiment packet v0.1.1
...
| Schema | `edcm.response-tension-release-experiment/0.1.1` |
| Candidate | `edcm.response-tension-release/0.1.0` |
| Canon selection | `null` |
| Supersedes | Immutable v0.1.0 packet SHA-256 `5e3fd9a24b6f13b9e40262b4d7daddb92ffbf098017d7ab2743817a1e03acaac` |

This complete v0.1.1 packet freezes the same single new experiment design and
supersedes v0.1.0 only to bind the repaired predecessor identity and frozen
pre-execution reproducibility controls. It does not modify the immutable
v0.1.0 bytes, authorize execution, alter EDCM equations, reopen prior evidence,
activate production, or select a canonical measurement. Packet v0.1.0 was
never executed; no candidate input, human label, partition membership,
per-event score, or sealed outcome was viewed between packet versions.

This v0.1.1 packet is immutable. Named people, keys, encrypted-bundle digests,
...
| PR head after evidence-binding review repair | `202feba979d462273b5df6cef4f4e5926a8a5a5b` |
| PR head tree | `e7979794981dec252637aecd56132c1e32a1149f` |
| Actions synthetic merge | `7db66c03a31b9c87de3315850e0b602bd4ae6f34` |
...
|---|---|---|
| [UCNS–EDCM joint experiments](https://github.com/The-Interdependency/edcm/actions/runs/30787168527) | Python 3.11/3.12 historical experiment tests and byte-identical v0.1–v0.4 reruns | Historical joint reports; not a raw-corpus rerun of the new holdout |
| [Skill and metadata compliance](https://github.com/The-Interdependency/edcm/actions/runs/30787168534) | Seven pinned skill-lib consumers clean; canonical metadata regeneration matched | Contract and drift evidence only |
| [CI](https://github.com/The-Interdependency/edcm/actions/runs/30787168528) | 239 tests passed with 20 producer-dependent skips on Python 3.11–3.13; integrity, authority boundaries, packaging, Twine, and clean-wheel smoke passed | Raw MultiWOZ bytes and event labels were absent; the sealed evaluation was not independently regenerated |
...
The repaired PR head additionally fails closed when the loaded EDCM runtime
does not match the recorded checkout, leaves a single-run byte-repeat finding
`not-evaluated`, and rejects colliding report, receipt, and atomic-temporary
paths before evaluation. Those repairs govern future v0.1.1 producer output.
They do not change the sealed v0.1.0 report or receipt bytes, their metrics, or
the controlling negative interpretation.
...
Two raters who are independent of the candidate author, every candidate
designer or implementer, and the external custodian label every event
independently while blinded to candidate scores and partition membership. A
third adjudicator subject to the same exclusions resolves disagreements. The
signed role manifest is frozen before any label is created; every rater and the
adjudicator is permanently excluded from candidate design and implementation
for this experiment.

Before adjudication, raw agreement must be at least `0.80` and unweighted
Cohen's kappa at least `0.70` overall. Both statistics are computed once on the
complete paired pre-adjudication three-category ledger (`resolved`,
`unresolved`, `not-adjudicable`), including rows later excluded as
`not-adjudicable`, before exclusion or allocation. Failure stops the
experiment; the manual may not be repaired after viewing agreement or
candidate results. The signed adjudicated ledger is controlling. Candidate
code has no authority to change it.
...
2. Name and sign the human-label authority, candidate-author and implementer,
   and external-custodian role manifests with the exclusions above.
...
| Labels | manual and training-example digests, excluded-role IDs, conflicts, blinding assertion, complete three-category agreement population, raw agreement, unweighted Cohen's kappa, adjudicator identity, pre- and post-adjudication ledger digests and signatures |
...
2. one contract-test file covering four synthetic tension transitions,
   ...
   partition isolation, equality at the `>=` threshold, pre-label seed binding,
   complete development-gate provenance, sealed ordered score-inventory
   digests, aggregate-only output, fail-closed behavior, and two-run
   determinism;
