...
- EDCM constructs words, then definitions, then recursive gonol relations while preserving identity, occurrence, order, multiplicity, relation identity, scale, source, and provenance; `edcm.character_word`, `edcm.definition_gonol`, and `edcm.recursive_gonol` are the implemented candidates for those stages;
