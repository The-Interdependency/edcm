...
Current EDCM text construction requires `characters -> words -> definitions -> recursive gonol relations`. This order is load-bearing. `edcm.gonol` is the implemented candidate constructor for those four stages. It is not selected canon, and construction does not activate measurement. Pronunciation is source metadata and does not alter the current construction. UCNS coupling geometry remains `hmmm` unless an explicit matching geometry authority is supplied.

Affixiation is defined by METAPAT. UCNS owns any exact geometric realization. EDCM applies affixiation to text-domain gonols. Affixiation is not another required construction stage; where exact source evidence authorizes it, it is represented as a recursive relation over eligible closed gonols. Each completed gonol closes before atomic participation at the next declared scale, without reopening, while constituent identity and provenance remain recoverable.
