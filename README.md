...
`full-stack` installs both the exact METAPAT producer and the EDCM UCNS profile producer. Use `ucns-experiments` only for the historical v0.1–v0.4 experiment epoch. Package availability alone attaches no evidence: UCNS activation also requires an exact tracked clean-checkout identity or a VCS installation whose installed files match their wheel-record hashes and whose cached bytecode matches those verified sources. Exact ordered `ucns_turns` are required before an observation is attached.
